        ...
        if (rand==1) {
            for (int i = 0; i < PlayingField.array[this.x][this.y].list.size(); i++) {
                ...
                if (!this.equals(PlayingField.array[this.x][this.y].list.get(i))) {
                    if (PlayingField.array[this.x][this.y].list.get(i) instanceof Mouse) {
                        Mouse victim = (Mouse) PlayingField.array[this.x][this.y].list.get(i);
            ...
            for (int i = 0; i < PlayingField.array[this.x][this.y].list.size(); i++) {
                    ...
                    if (PlayingField.array[this.x][this.y].list.get(i) instanceof Caterpillar) {
                        ...
                        if (PlayingField.PROBABILITIES[key][keyVictim] > 0) {
                            ...
                            int chance = (int) (Math.random() * PlayingField.PROBABILITIES[key][keyVictim]);
                            if (chance > 0 && chance <= PlayingField.PROBABILITIES[key][keyVictim]) {
                                this.isFed++; //животное поело
